...
#     On the body or in the project settings
...
# TODO: change the way max speed is enforced to allow speeds higher than the max
...
func do_movement(state):
	...
	if id.length() > 0.1:
		...
		body.apply_central_impulse(move_impulse)
